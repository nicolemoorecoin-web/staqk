...
import {
  FiChevronRight, FiBell, FiRefreshCw, FiGift, FiSettings,
  FiHelpCircle, FiGlobe, FiDollarSign, FiGrid,
} from 'react-icons/fi';

// (optional) keeps this page from being statically exported,
// but the important part is: never touch localStorage outside effects.
export const dynamic = 'force-dynamic';

export default function MePage() {
  // Start with harmless defaults (no localStorage here)
  const [user, setUser] = useState({ name: 'Henry', email: 'henry@example.com', avatar: null });
  ...
  useEffect(() => {
    ...
    try {
      const savedName = localStorage.getItem('staqk_user_name');
      if (savedName) setUser((u) => ({ ...u, name: savedName }));

      const savedDark = localStorage.getItem('pref.darkMode');
      if (savedDark != null) setDarkMode(savedDark === 'true');

      const savedLang = localStorage.getItem('pref.language') || 'English';
      setLanguage(savedLang);

      const fiat = localStorage.getItem('pref.fiat') || 'USD';
      const crypto = localStorage.getItem('pref.crypto') || 'BTC';
      setCurrencies(`${fiat} & ${crypto}`);
    } catch {}
  }, []);

  // Persist dark mode & update <html> class after mount
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      ...
      <div className="max-w-xl mx-auto px-4 pt-8">
        ...
        <section className="bg-[#151a28] rounded-2xl border border-blue-900/30 p-5 shadow-xl">
          <Link href="/profile/edit" className="flex items-center gap-4">
            {user.avatar ? (
              <img src={user.avatar} alt="avatar" className="w-14 h-14 rounded-full object-cover border-2 border-blue-500/70" />
            ) : (
              <HiUserCircle className="w-14 h-14 text-blue-400" />
            )}
            <div className="min-w-0">
              <h1 className="text-2xl font-extrabold text-white truncate">Hi, {user.name}</h1>
          ...
          <div className="mt-5 bg-[#11182a] rounded-2xl px-4 py-3 shadow-inner border border-blue-900/20">
            <div className="flex items-start gap-2">
              ...
              <p className="text-gray-200 text-sm leading-relaxed">
                Ethereum (ETH) price is approaching <span className="font-semibold">$4,300</span>
              </p>
            </div>
            <Link href="/notifications" className="mt-3 block w-full text-center text-blue-300 font-semibold">
              See All
            </Link>
          </div>
        ...
        <section className="mt-6 bg-[#151a28] rounded-2xl border border-blue-900/30 shadow-xl overflow-hidden">
          ...
          <ul className="divide-y divide-blue-900/20">
            <ToggleRow title="Dark Mode" value={darkMode} onChange={() => setDarkMode((v) => !v)} />
            <LinkRow title="Language" subtitle={language} icon={<FiGlobe className="text-gray-400" />} href="/settings/language" />
            <LinkRow title="Default Currencies" subtitle={currencies} icon={<FiDollarSign className="text-gray-400" />} href="/settings/currency" />
          </ul>
        </section>
