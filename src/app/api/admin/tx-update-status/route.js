...
export async function POST(req) {
  ...
  const { txId, status, reason } = await req.json().catch(() => ({}));
  if (!txId || !["SUCCESS", "FAILED"].includes(status)) {
  ...
  if (tx.status !== TxStatus.PENDING) {
    return NextResponse.json({ error: `Cannot update tx in status ${tx.status}` }, { status: 409 });
  }
  ...
  if (status === "SUCCESS") {
    ...
    return NextResponse.json({ ok: true, via: "proxy-approve" });
  } else {
    await prisma.tx.update({
      where: { id: tx.id },
      data: { status: TxStatus.FAILED, notes: reason ?? tx.notes },
    });
    return NextResponse.json({ ok: true, via: "proxy-reject" });
  }
}
