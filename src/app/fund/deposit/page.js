...
const demoTx = [
  { type: "Deposit", amount: "+$1,000", method: "Bank Transfer", status: "Successful" },
  { type: "Deposit", amount: "+$2,000", method: "Credit Card", status: "Successful" },
];
  ...
  return (
    <main className="bg-[#10141c] min-h-screen py-8 px-2">
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-800 sticky top-0 bg-[#10141c]/95 z-10">
        ...
        <h1 className="text-white font-bold text-lg">Deposit Funds</h1>
        <div className="w-6" />
      </div>

      {/* Balance Card */}
      <section className="max-w-lg mx-auto mb-6">
        <div className="bg-gradient-to-tr from-green-700 to-green-500 rounded-2xl p-6 shadow-lg">
        ...
        {methods.map((m) => (
          <button
            ...
            className={`flex-1 flex flex-col items-center py-3 rounded-lg font-semibold ${
              method === m.value ? "bg-green-700 text-white" : "bg-[#222842] text-gray-300"
            }`}
          >
            {m.icon}
      ...
      <form onSubmit={handleSubmit} className="bg-[#181d29] rounded-2xl p-6 max-w-lg mx-auto shadow-lg">
        {success && (
          <div className="mb-4 flex items-center gap-2 bg-green-800 text-green-300 p-2 rounded-lg">
            <FaCheckCircle /> Deposit Successful!
          </div>
        )}
        <label className="text-white font-bold block mb-2">Amount to Deposit</label>
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={handleMax}
            className="px-3 py-2 bg-green-700 rounded-lg text-white font-bold"
          >
            Max
          </button>
        </div>
        <button
          type="submit"
          className="w-full mt-4 py-3 bg-green-700 hover:bg-green-800 text-white font-bold rounded-lg"
        >
      ...
      <section className="max-w-lg mx-auto mt-8">
        <h3 className="text-lg font-bold text-white mb-3">Recent Deposits</h3>
        <div className="bg-[#181d29] rounded-lg p-4">
          {demoTx.map((tx, i) => (
            <div key={i} className="flex justify-between text-white mb-2 last:mb-0">
              <span>{tx.method}</span><span className="text-green-400">{tx.amount}</span>
